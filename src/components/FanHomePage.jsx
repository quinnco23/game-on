...
import { getPublicGames, getUpcomingGames } from "../services/gamesService";

function getPlayerOfGame(game) {
  const state = game?.state ?? game?.game_state ?? {};

  const lineups = state.lineups ?? {};

  const batterStats = state.stats?.batters ?? {};
...
export function FanHomeScreen() {
  ...
  const liveGames = games.filter((game) => game.status === "scoring");

  const finalGames = games.filter((game) => game.status === "final");

  if (loading) {
    return <main className="p-4">Loading games...</main>;
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <header>
        ...
        <h1 className="scoreboard-title mt-1 text-3xl">Scores</h1>
      </header>
    ...
    "
        >
          <h2 className="scoreboard-title text-xl">Upcoming Games</h2>

          <span className="scoreboard-label opacity-60">Schedule</span>
        </div>
...
{(game.venue || game.field_name) && (
  <div className="scoreboard-label mt-2 opacity-60">
    {game.venue}
    ...
    {game.field_name}
      ...
      <section className="space-y-3">
        ...
        "
        >
          <h2 className="scoreboard-title text-xl">Live</h2>
            ...
            {liveGames.map((game) => (
              ...
              >
                <GameScoreCard
                  game={game}
                  status="live"
                  onClick={() => navigate(`/fan/games/${game.id}`)}
                />
              </div>
            ))}
          </div>
      ...
      <section className="space-y-3">
        ...
        "
        >
          <h2 className="scoreboard-title text-xl">Final</h2>

          <div className="scoreboard-label opacity-60">Recent Games</div>
        </div>
